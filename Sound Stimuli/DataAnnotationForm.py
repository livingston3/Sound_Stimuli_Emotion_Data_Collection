...
file_path_dict = {"Animal Sound":"./IADS-E/IADS-E sound stimuli (IADS-2 is not included)/Animal",  
				  "Nature Sound":"./IADS-E/IADS-E sound stimuli (IADS-2 is not included)/Nature", 
				  "People Sound":"./IADS-E/IADS-E sound stimuli (IADS-2 is not included)/People",
				  "Transport Sound":"./IADS-E/IADS-E sound stimuli (IADS-2 is not included)/Transport",
...
def play():
	...
	wave_obj = sa.WaveObject.from_wave_file(path + "/" + sound_list[idx]) #input combined path of soundfile
...
if __name__ == "__main__":
	...
	place_holder = Label(root,text="").grid(row=2,column=0)
	place_holder = Label(root,text="").grid(row=3,column=0)
	...
	w3 = OptionMenu(root, sound_group, "Practice Sound","Animal Sound","Nature Sound",
										"People Sound", "Transport Sound", command = get_sound_group)
	...
	play_button.grid(row=4, column=1,ipadx="40")

	#create Soundlabel and position
	sound_label = Label (root, text= f'Play sound {idx + 1} of 30' , fg="Black", bg="Light Blue")
	sound_label.grid(row=4, column=0)
	place_holder2 = Label(root,text="").grid(row=5,column=0)
	...
	w3.grid(row=6, column=1, ipadx="30")

	#Create emotion dropdown label
	emotion_label = Label (root,text= f'Select emotion' , fg="Black", bg="Light Blue")
	emotion_label.grid(row=6,column=0,)
	place_holder3 = Label(root,text="").grid(row=7,column=0)
	...
	w4.grid(row=8, column=1, ipadx="30")

	#create emotion rating dropdown label
	emotion_rating_label = Label (root,text= 'Emotion Intensity' , fg="Black", bg="Light Blue")
	emotion_rating_label.grid(row=8,column=0)

	# call excel function
	excel()

	# create a Submit Button and place into the root window
	placeholder4 = Label(root, text="").grid(row=9,column=0)
	placeholder5 = Label(root, text="").grid(row=10,column=0)
